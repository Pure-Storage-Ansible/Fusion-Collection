...
options:
  ...
  state:
    ...
    - Currently there is no mechanism to delete an array
    default: present
    choices: [ present ]
    type: str
  display_name:
...
def wait_operation_finish(module, op_id, fusion):
    ...
    while True:
        ...
        if op.status == "Succeeded" or op.status == "Failed":
            return op
        time.sleep(op.retry_in_millis / 1000)
...
def main():
    ...
    argument_spec.update(
        dict(
            ...
            hardware_type=dict(type="str", required=True),
